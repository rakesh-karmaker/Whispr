...
function AssetsLayout({
  ...
  return (
    <div className="w-full flex flex-col">
      <button
        type="button"
        className={`w-full flex justify-between items-center cursor-pointer hover:bg-white-2 dark:hover:bg-d-light-dark-gray transition-all pr-2 duration-200 rounded-md ${
          open ? "bg-white-2 dark:bg-d-light-dark-gray" : ""
        }`}
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <p className="flex flex-col items-start">
            <span className="font-semibold text-lg dark:text-d-white/90 tracking-wider">
              {title}
            </span>
            <span className="text-gray-500 text-sm dark:text-d-white/50">
              {subTitle}
            </span>
          </p>
        </div>
        <FaChevronDown
          className={`text-lg text-gray-500 dark:text-d-gray/50 transition-all duration-200 hover:text-black cursor-pointer ${
            open ? "rotate-180" : ""
          }`}
        />
      </button>
